...
const PARTNER_DOMAINS = (process.env.PARTNER_DOMAINS || "")
  .split(",")
...
/**
 * Returns true if the given URL's hostname matches any partner domain.
 * Subdomain-aware: sub.spinjo.com matches spinjo.com
 */
function isPartnerDomain(url) {
  try {
    const hostname = new URL(url).hostname.toLowerCase();
    return PARTNER_DOMAINS.some(
 ...
 * Stops early if the current URL's domain matches a partner domain.
 * Returns { finalUrl, status, hops, found }.
 */
async function traceRedirectsOnce(startUrl, proxyUrl) {
  ...
  // Check if the starting URL is already a partner domain
  if (isPartnerDomain(currentUrl)) {
  ...
  while (true) {
    ...
    if (!REDIRECT_CODES.has(statusCode)) {
      return { finalUrl: currentUrl, status: statusCode, hops, found: isPartnerDomain(currentUrl) };
    }
    ...
    if (!location) {
      return { finalUrl: currentUrl, status: statusCode, hops: hops - 1, found: isPartnerDomain(currentUrl) };
    }

    referer = currentUrl;
    currentUrl = new URL(location, currentUrl).href;

    // Check each hop — stop early if partner domain matched
    if (isPartnerDomain(currentUrl)) {
...
app.get("/", (_req, res) => {
  res.json({
    service: "partnerDomainCheckerAPI",
...
app.listen(PORT, () => {
  console.log(`partnerDomainCheckerAPI listening on http://localhost:${PORT}`);
});
